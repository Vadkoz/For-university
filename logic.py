...
for i in query:
    if ord(i) in my_ascii:
        vars.append(i)
vars = list(set(vars))
to_ones = subsets(vars)
vars_values = [0] * len(vars)
all_cases = []
all_queries = []
for subset in to_ones:
    vars_values = [0] * len(vars)
    for j in subset:
        vars_values[vars.index(j)] = 1
    all_cases.append(vars_values)

for i in all_cases:
    query2 = query
    for n, j in enumerate(i):
        query2 = query2.replace(vars[n], str(j))
...
for i in range(len(all_queries)):
    print(str(all_cases[i]) + '_'*len(vars)*3 + str(results[i]))
